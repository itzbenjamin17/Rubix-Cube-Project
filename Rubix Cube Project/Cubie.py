...
class Cubie:
    ...
    def rotateVertices(self, rotationMatrix: np.array):
        '''Rotate the vertices of the cubie using the given rotation matrix'''
        newVertices = []
    ...
    def assignFaces(self, colours: dict):
        '''Assign the correct colours to each face of the cubie'''
        faces = {}
    ...
    def moveFaces(self,faceMapping: dict):
        ...
        for i,face in self.faces.items():
                    if face.getName() in faceMapping:
                       newFaceName = faceMapping[face.getName()]
                       newFaces[faceNames[newFaceName]] = Face(face.colour,newFaceName)
                    else:
